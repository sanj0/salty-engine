...
/**
 * A <code>GameObject</code> can be added to a {@link de.edgelord.saltyengine.scene.Scene}
 * using {@link de.edgelord.saltyengine.scene.Scene#addGameObject(GameObject)}.
 * It will then have collision detection with all the other
 * <code>GameObject</code>s in the
 * <code>Scene</code>, it will be drawn using
 * {@link #draw(SaltyGraphics)}, initialized using {@link #initialize()} and
 * updated every fixed tick using {@link #onFixedTick()}.
 * <p>
 * For more information, please visit the project's wiki on
 * https://www.github.com/edgelord314/salty-engine/wiki
 */
...
public abstract class GameObject extends ComponentContainer implements Drawable, FixedTickRoutine, CollideAble, InitializeAble {
    ...
    /**
     * The name of the default {@link SimplePhysicsComponent} {@link
     * #physicsComponent}.
     */
    ...
    /**
     * The List of collisions that has been occurred in the last collision
     * detection, used for {@link #onCollisionDetectionFinish(List)}.
     */
    private final List<CollisionEvent> collisions = new CopyOnWriteArrayList<>();
    /**
     * The default physics component, used for stopping the
     * <code>GameObject</code> when a collision in a direction occurs and to
     * accelerate it using e.g. {@link #accelerate(float, Directions.Direction)}
     * or {@link #accelerateTo(float, Directions.Direction)}.
     */
    private final SimplePhysicsComponent physicsComponent;
    /**
     * Used for internal purposes to make sure that the list of {@link
     * #collisions} is cleared once per collision detection.
     */
    private boolean clearCollisions = true;
    /**
     * The collider of this <code>GameObject</code>, used to check collisions
     * between this and other <code>GameObject</code>s.
     */
    private Collider collider;

    /**
     * Used for internal purposes to call {@link #onCursorEnters()} and {@link
     * #onCursorExits()}.
     */
    private boolean cursorAlreadyTouching = false;

    /**
     * If this is true, the physics of any other GameObject in the {@link
     * de.edgelord.saltyengine.scene.Scene} will ignore this one on collision,
     * but the collision is still detected.
     */
     ...
     * is accelerate and slowed down, but currently not the behaviour when it
     * collides.
     */
    private float mass = 1f;

    /**
     * When this is <code>false</code>, {@link #initialize()} will be called on
     * the next fixed tick. <br> Can be set to
    ...
    /**
     * Initializes this <code>GameObject</code>. The advantage over doing so in
     * the constructor is that, in many cases, the constructor will be called
     * while the previous {@link de.edgelord.saltyengine.scene.Scene} is still
     * active, so e.g. a {@link de.edgelord.saltyengine.components.gfx.LightComponent}
     * would add it's {@link de.edgelord.saltyengine.graphics.light.Light} to
     * the wrong <code>Scene</code>. This method is called every fixed tick when
    ...
    /**
     * This method is called every fixed tick for every <code>GameObject</code>
     * that touches this one.
     *
     * @param event the <code>CollisionEvent</code> with any necessary
     *              information to handle a collision
     */
    @Override
     ...
     * wiki at https://www.github.com/edgelord314/salty-engine/wiki for more
     * information.
     *
    ...
    /**
     * This method is called every time the {@link de.edgelord.saltyengine.scene.Scene}
     * is repainted. <br> With this method, you can draw anything using the
     * given {@link SaltyGraphics}.
     *
    ...
    /**
     * This is called every time the cursor enters the {@link #getTransform()}
     * of this
     * <code>GameObject</code>. As this takes
     * quite a bit of performance, you can disable/enable this feature using
     * {@link de.edgelord.saltyengine.utils.SaltySystem#gameObjectMouseEventsAgent}.
     */
    public void onCursorEnters() {
    }

    /**
     * This is called every time the cursor exits the {@link #getTransform()} of
     * this
     * <code>GameObject</code>. As this takes
     * quite a bit of performance, you can disable/enable this feature using
     * {@link de.edgelord.saltyengine.utils.SaltySystem#gameObjectMouseEventsAgent}.
    ...
    /**
     * This is called every time the collision detection of this
     * <code>GameObject</code> is done. <br> It is especially useful for e.g.
     * processing if this <code>GameObject</code> hits any other
     * <code>GameObject</code> with a specific tag.
     *
    ...
    /**
     * This method is used internally to call {@link #onFixedTick()} and do some
     * stuff with the physics as well as {@link Hitbox#recalculate() updating}
     * the {@link #hitbox}.
     */
    ...
    /**
     * This is used internally to handle the event of the cursor entering this
    ...
    /**
     * This is used internally to handle the event of the cursor exiting this
     * <code>GameObject</code>.
     */
    ...
    /**
     * This method sets the {@link Force#getAcceleration() acceleration} of the
     * default force in the given direction to the given acceleration. On the
     * next fixed tick, this acceleration is reset to 0f. This is the
     * recommended way fro player control in a few cases because the momentum of
     * this GameObject will slowly fade out and so the controls aren't precise.
     * However, this might be useful for some physics-related games.
     *
     * @param acceleration the acceleration to be set to the specific default
     *                     force
    ...
    /**
     * Calls {@link #accelerate(float, Directions.Direction)} for every {@link
     * de.edgelord.saltyengine.utils.Directions.Direction} that the given {@link
     * Directions} has.
     *
     * @param acceleration the target acceleration for all directions that the
     *                     given <code>Directions</code> has.
    ...
    /**
     * This method sets the {@link Force#getVelocity() velocity} of the default
     * velocity force with the given direction to the given value. This velocity
     * only rests for one tick. This is the recommended way for player control
     * in most cases because it's more precise than working with acceleration.
     *
    ...
    /**
     * Calls {@link #accelerateTo(float, Directions.Direction)} for every {@link
     * de.edgelord.saltyengine.utils.Directions.Direction} that the given {@link
     * Directions} has.
     *
     * @param velocity   the target velocity for all directions that the given
     *                   <code>Directions</code> has
    ...
    /**
     * Returns whether the cursor touches the {@link #getTransform() transform}
     * of this
     * <code>GameObject</code> or not.
     *
     * @return <code>true</code> if the cursor
     * touches the {@link #getTransform() transform} of this
     * <code>GameObject</code> and <code>false</code> if not.
     */
    ...
    /**
     * Returns the {@link #hitbox} as a {@link SimpleHitbox}, as it is by
     * default.
     *
     * @return the return value of {@link #getHitbox()} casted to {@link
     * SimpleHitbox}
     */
