...
public class ExampleClient extends SaltyClient {

    public ExampleClient(String serverIPAddress) throws SocketException, UnknownHostException {
    ...
    public void handleIncoming(String data, InetAddress ipAddress, int port) {
        ...
        if (data.startsWith("0")) {
            ...
            try {
                newPlayer = new Player(Float.valueOf(capsules[2]), Float.valueOf(capsules[3]), 100, 100, capsules[1], InetAddress.getByName(capsules[4]), Integer.valueOf(capsules[5]));
            } catch (UnknownHostException e) {
                e.printStackTrace();
            }

            addEntity(newPlayer);
            SceneManager.getCurrentScene().addGameObject(newPlayer);

            // if the message starts with 1 it means an entity repositioning.
        } else if (data.startsWith("1")) {
            String[] capsules = data.split(":");

            if (!capsules[1].equals(ClientExample.username))
