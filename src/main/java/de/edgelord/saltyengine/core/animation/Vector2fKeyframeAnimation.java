...
/**
 * Packages two {@link KeyframeAnimation}s into one for animating positions
 * based on {@link de.edgelord.saltyengine.transform.Vector2f}s.
 */
public class Vector2fKeyframeAnimation {
    ...
    /**
     * Constructs a new instance by setting {@link #animationX} and {@link
     * #animationY} to the two given ones, respectively.
     *
    ...
    /**
     * Returns the {@link KeyframeAnimation#nextDelta()} of both x and y
     * animations stored in a {@link Vector2f} instance. For animating, one
     * would want to add the returned vector to the object's position {@code
     * object.getPosition().add(animation.nextDelta())}
     *
     * @return the next delta of the 2D movement stored in a {@link Vector2f}
     */
    ...
    /**
     * Adds a new Keyframe to the animation at the given timecode with the given
    ...
    /**
     * Adds a new Keyframe to the animation at the given timecode with the same,
     * given value for both x and y components of this animation.
     *
     * @param timecode the timecode of the new Keyframe
    ...
    /**
     * Calculates the animation in its current state by calling the according
     * method for both individual KeyframeAnimations.
     *
    ...
    /**
     * Checks whether or not this animation has reached its end, which is the
     * case when both individual animations have ended.
     *
    ...
    /**
     * Adds the given Keyframe to both individual animations
     *
    ...
    /**
     * Removes the given Keyframe from both individual animations
     *
    ...
    /**
     * Removes the Keyframe at the given timecode from both individual
     * animations.
     *
    ...
    /**
     * Removes all Keyframes in both animations that have the given value.
     *
    ...
    /**
     * Restarts the animation by restarting both individual animations.
     *
    ...
    /**
     * Sets the current frame to the given one in both individual animations
     *
